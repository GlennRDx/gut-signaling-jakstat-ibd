...
#' @param dataset_name Dataset identifier (e.g., "01_IL_EPI", "02_CO_IMM", etc.)
...
#'
#' @examples
#' # Example usage for ileum epithelial cells:
...
#'   condition1 = "Control",
#'   condition2 = "ActiveCD",
#'   output_dir = "/home/glennrdx/Documents/gut-signaling-jakstat-ibd/05_results_repository/",
#'   dataset_name = "01_IL_EPI"
#' )
#'
...
#'   output_dir = "/home/glennrdx/Documents/gut-signaling-jakstat-ibd/05_results_repository/",
#'   dataset_name = "02_CO_STR",
#'   return_results = TRUE
#' )
run_deseq2_comparison <- function(seurat_obj, condition_col, sample_col, celltype_col, 
                                  condition1, condition2, output_dir, dataset_name, 
                                  return_results = FALSE) {
  
  # Parse dataset name to extract components
  dataset_parts <- strsplit(dataset_name, "_")[[1]]
  if(length(dataset_parts) != 3) {
    stop("Dataset name must be in format 'ID_LOCATION_CELLGROUP' (e.g., '01_IL_EPI')")
  }
  
  dataset_id <- dataset_parts[1]
  location_code <- dataset_parts[2]
  cellgroup_code <- dataset_parts[3]
  
  # Map location codes to full names
  location_mapping <- c("CO" = "Colon", "IL" = "Ileum")
  location_full <- location_mapping[location_code]
  
  # Map cell group codes to full names
  cellgroup_mapping <- c("EPI" = "EPI", "IMM" = "IMM", "STR" = "STR")
  cellgroup_full <- cellgroup_mapping[cellgroup_code]
  
  if(is.na(location_full) || is.na(cellgroup_full)) {
    stop("Invalid location or cellgroup code. Use CO/IL for location and EPI/IMM/STR for cellgroup")
  }
  ...
  # Create results directories following the new structure
  # Structure: base_dir/Comparison/Location/CellGroup/Dataset_ID/
  results_base_dir <- file.path(output_dir, comparison_name, location_full, cellgroup_full, paste0("dataset_", dataset_id))
  ...
  create_pca_plot <- function(dds, cell_type, comparison, output_dir) {
    ...
    pca_plot <- ggplot(pca_data, aes(x = PC1, y = PC2, color = condition, shape = condition)) +
      ...
      ggtitle(paste0("PCA Plot: ", cell_type, " - ", comparison, "\n", 
                     location_full, " ", cellgroup_full, " (Dataset: ", dataset_name, ")")) +
      theme_minimal()
    
    pca_file <- file.path(output_dir, paste0(dataset_name, "_", gsub("[/\\\\?%*:|\"<> ]", "_", cell_type), "_PCA_plot.pdf"))
  ...
  create_dispersion_plot <- function(dds, cell_type, comparison, output_dir) {
    disp_file <- file.path(output_dir, paste0(dataset_name, "_", gsub("[/\\\\?%*:|\"<> ]", "_", cell_type), "_Dispersion_plot.pdf"))
  ...
  create_ma_plot <- function(res, cell_type, comparison, output_dir) {
    ma_file <- file.path(output_dir, paste0(dataset_name, "_", gsub("[/\\\\?%*:|\"<> ]", "_", cell_type), "_MA_plot.pdf"))
    pdf(ma_file, width = 10, height = 8)
    plotMA(res, main = paste0("MA Plot: ", cell_type, " - ", comparison, "\n",
                              location_full, " ", cellgroup_full, " (Dataset: ", dataset_name, ")"), 
           ylim = c(-5, 5), cex = 0.8)
  ...
  perform_DESeq2_celltype <- function(counts_matrix, metadata, condition1, condition2, cell_type) {
    ...
    safe_cell_type <- gsub("[/\\\\?%*:|\"<> ]", "_", cell_type)
    volcano_file <- file.path(output_dir, paste0(dataset_name, "_", safe_cell_type, "_Volcano_plot.pdf"))
    ...
    # Save results table with dataset prefix
    output_file <- file.path(output_dir, paste0(dataset_name, "_", safe_cell_type, "_DEG_results.csv"))
  ...
  # Save summary table with dataset prefix
  summary_file <- file.path(results_base_dir, paste0(dataset_name, "_DEG_summary_by_celltype.csv"))
  write.csv(summary_table, summary_file, row.names = FALSE)
  
  # Print summary
  cat("\nAnalysis Summary:\n")
  cat("Dataset:", dataset_name, "\n")
  cat("Location:", location_full, "\n")
  cat("Cell Group:", cellgroup_full, "\n")
  ...
  if(nrow(successful_results) > 0) {
    ...
    print(top_results[, c("dataset", "location", "cellgroup", "cell_type", "upregulated", "downregulated", "total_significant")])
  }
